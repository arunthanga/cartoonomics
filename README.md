...
(this resolves open question Q5).
...
| Path | Role |
|---|---|
| `src/cartoonomics/connectors/` | Polite, provenance-capturing source connectors (`base`, offline `fixture`, reference `NSE`/`BSE`). |
| `src/cartoonomics/parsing/` | Read PDF (pdfplumber) and delimited documents into a canonical intermediate. |
| `src/cartoonomics/analysis/` | Metrics (XIRR) and the cashflow → CartoonSpec builder. |
| `src/cartoonomics/format/` | **CartoonSpec** — the predefined, versioned exchange format (pydantic). |
| `src/cartoonomics/pipeline.py` | Wires it all together end-to-end. |
| `src/cartoonomics/config.py`, `tdd.py` | The switchable **TDD mode**. |
| `web/` | A framework-free **animated renderer** that consumes a CartoonSpec. |
| `tests/unit/`, `tests/regression/` | The unit and regression test suites. |
...
make demo                          # writes web/cartoon_spec.json
...
- **Unit** (`tests/unit/`, `-m unit`): fast, isolated tests of each module —
...
- **Regression** (`tests/regression/`, `-m regression`): guards the CartoonSpec
  contract — a **golden-file** test for the demo payload plus property-based
  invariants (e.g. net change == sum of activity totals). Regenerate the golden
  intentionally with `python tests/regression/_regen_golden.py`.
